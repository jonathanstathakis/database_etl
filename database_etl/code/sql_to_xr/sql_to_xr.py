...
def fetch_imgs(con: db.DuckDBPyConnection) -> list[pd.DataFrame]:
    ...
    return [
        pd.read_parquet(path).set_index("time").rename_axis("wavelength", axis=1)
        for path in paths
    ]
...
def get_imgs_as_dict(con: db.DuckDBPyConnection) -> dict:
    ...
    time_aligned_imgs = align_df_times(imgs, m=7800)
    return {img["id"][0]: img.drop("id", axis=1) for img in time_aligned_imgs}


def load_xr_dset(con: db.DuckDBPyConnection) -> xr.Dataset:
    """
