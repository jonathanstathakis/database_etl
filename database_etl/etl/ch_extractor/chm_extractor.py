...
def get_data(path: Path) -> tuple[pl.DataFrame, pl.DataFrame]:
    ...
    data = (
        ...
        .reset_index()
        .pipe(pl.from_pandas)
        .with_columns(pl.col("mins").sub(pl.col("mins").first()))
        .with_columns(
            pl.lit(datadir.metadata["id"]).alias("id"), pl.exclude(["id", "mins"])
        )
        .select("id", "mins", pl.exclude(["id", "mins"]))
    )
...
def write_extraction_dir(extract_path_prefix: Path, data: pl.DataFrame, metadata: pl.DataFrame) -> str:
    """
...
def extract_run_data(path: Path, overwrite: bool = False) -> str:
    """
    Extract the metadata and data of a single .D found at  `path` as parquets stored
    within the .D file under "extract_<current datetime>".
