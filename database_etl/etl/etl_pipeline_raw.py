...
def get_metadata_as_dict(df: pd.DataFrame) -> dict[str, pl.DataFrame]:
    df_dict = df.set_index("runid").to_dict(orient="index")

    # to_dict returns ambiguous 'Hashable', need to tell typechecker that
    # the keys are explicitly str.
...
def get_data(
...
) -> xr.Dataset | list[tuple[pl.DataFrame, pl.DataFrame]] | None:
    """
    ...
    metadata_df: pl.DataFrame = get_sample_metadata(con=con)

    match output:
        case "xr":
            imgs_as_dict: dict[str, pl.DataFrame] = get_imgs_as_dict_numeric_cols_only(
                imgs=imgs
            )
            metadata_as_dict: dict[str, pl.DataFrame] = get_metadata_as_dict(
                df=metadata_df.to_pandas()
            )
